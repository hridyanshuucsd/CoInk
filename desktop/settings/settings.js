...
const translations = {
  zh:{
    eyebrow:"几分钟内连接 AI 画布", title:"设置你的 AI 连接", subtitle:"CoInk 在本机运行。选择 AI 模型来源，测试一次，然后就可以开始创作。",
    ...
    kimiPartnerTitle:"CoInk 是 Kimi 2026 开源合作伙伴", otherProviders:"其他模型来源", kimiCodeLink:"Kimi Code", kimiChinaLink:"中国大陆", kimiGlobalLink:"全球", kimiOffering:"Kimi 服务", kimiRegion:"访问区域", kimiCodeOption:"Kimi Code · 订阅制", kimiPlatformOption:"开放平台 · 按量付费", globalOption:"全球", chinaOption:"中国大陆",
    apiType:"API 类型", model:"模型", baseUrl:"API 地址", apiKey:"API Key", savedLocally:"已保存在本机", apiKeyHelp:"保存在仅当前用户可读的本地凭据文件中，不会暴露给画布页面。",
    kimiCliRequired:"使用你已有的 Kimi Code CLI 登录状态。", kimiCliInstallHelp:"CoInk 可以安装官方 Kimi Code CLI，并使用它已有的登录状态。", installGuide:"安装说明", codexRequired:"通过 Codex CLI 使用你现有的 ChatGPT 登录状态。", codexInstallHelp:"CoInk 会安装官方 CLI，并直接使用你已有的登录状态。", claudeRequired:"通过 Claude Code 使用你现有的 Anthropic 登录状态。", claudeInstallHelp:"CoInk 会安装官方稳定版 CLI，并直接使用你已有的登录状态。", install:"安装", modelOptional:"模型（可选）", pathOptional:"可执行文件路径（可选）",
    ...
    lanAccessTitle:"在其他设备上打开 CoInk", lanAccessHelp:"在连接同一可信网络的设备上使用以下地址，并保持 CoInk 运行。", lanDynamicPort:"启动后系统会选择端口，并显示最终的局域网地址。", noLanAddress:"暂未检测到局域网地址。连接网络后，启动时会显示最终地址。", copyAddress:"复制地址",
    ...
    testing:"正在测试连接", testingBody:"正在本地保存设置并检查模型连接……", testTimedOut:"连接测试已超时", testTimedOutBody:"设置已保存。连接测试超过 30 秒，你仍然可以启动 CoInk 进入画布。", installing:"正在安装", installingBody:"正在从官方来源下载并校验，请稍候……", installed:"安装完成", checkingExistingSession:"正在检查现有账户登录状态……", installFailed:"自动安装未完成", success:"连接成功", launching:"CoInk 即将启动。", savedTestFailed:"设置已保存，但连接测试失败", launchAnyway:"仍然启动", launchingSaved:"正在使用已保存的设置启动 CoInk……", failed:"尚未连接", unexpected:"无法打开桌面设置接口。",
  },
};
...
async function testAndLaunch() {
  ...
  try {
    ...
    if (!result.ok) {
      ...
      if (result.timedOut) {
        setStatus("warning",
          ...
          translations[currentLanguage]?.testTimedOutBody || "Settings were saved. The connection test exceeded 30 seconds; you can still launch CoInk and enter the canvas.",
          action);
        return;
      }
    ...
    setStatus("success", translations[currentLanguage]?.success || "Connection ready", `${result.message} ${translations[currentLanguage]?.launching || "CoInk will launch now."}`);
    const launched = await desktop.launch();
    if (!launched?.ok) setStatus("error", translations[currentLanguage]?.failed || "Unable to launch", launched?.error || "Restart CoInk to use the saved settings.");
  } catch (error) {
...
async function launchSavedSettings() {
  setBusy(true);
  setStatus("loading", translations[currentLanguage]?.launching || "Launching CoInk", translations[currentLanguage]?.launchingSaved || "Starting CoInk with your saved settings…");
  try {
    const launched = await desktop.launch();
    if (!launched?.ok) setStatus("error", translations[currentLanguage]?.failed || "Unable to launch", launched?.error || "Restart CoInk to use the saved settings.");
  } catch (error) {
