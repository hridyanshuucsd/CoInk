...
const UPDATE_OWNER = "hridyanshuucsd";
const UPDATE_REPOSITORY = "CoInk";
...
function compareVersions(left, right) {
  ...
  if (!a || !b) throw new Error("The release service returned an invalid CoInk version.");
...
function expectedAssetName(platform, arch, version) {
  if (platform === "darwin" && ["arm64", "x64"].includes(arch)) return `CoInk-${version}-mac-${arch}.zip`;
  if (platform === "win32" && ["arm64", "x64"].includes(arch)) return `CoInk-Setup-${version}-win-${arch}.exe`;
  return "";
}
...
async function installMacUpdate(options) {
  ...
  if (!target || path.basename(target) !== "CoInk.app") throw new Error("CoInk must be run from an installed CoInk.app before it can update itself.");
  ...
  const directory = path.dirname(options.downloadedPath),
    ...
    candidate = path.join(staging, "CoInk.app"),
    helper = path.join(directory, `install-${token}.sh`),
    backup = `${target}.coink-backup-${token}`,
  ...
  try {
    ...
    if (identifier !== "io.github.hridyanshuucsd.coink") throw new Error("The update archive is not a CoInk desktop application.");
...
function createUpdateManager(options) {
  ...
  function publish(status, updates = {}) {
    ...
    try { onStateChange(getState()); } catch (error) { logger.warn?.(`CoInk update UI failed: ${error.message || error}`); }
    return getState();
  }
    ...
    try {
      const response = await fetchImpl(RELEASE_API_URL, {
        headers:{
          ...
          "User-Agent":`CoInk/${currentVersion}`,
      ...
      return {
        version,
        title:String(value?.name || `CoInk ${version}`).trim().slice(0, 200),
  ...
  async function check(manual = false) {
    ...
    try {
      ...
      if (!asset) throw new Error(`CoInk v${latest.version} does not include ${expectedAssetName(platform, arch, latest.version) || "a compatible installer"}.`);
      publish("available", {
      ...
      logger.warn?.(`CoInk update check failed: ${error.message || error}`);
      publish("error", {
        visible:Boolean(manual),
        error:`CoInk could not check GitHub Releases. ${String(error?.message || error || "").trim()}`.trim().slice(0, 500),
      });
      return false;
    } finally {
  ...
  async function download() {
    ...
    if (!asset) {
      publish("error", { visible:true, error:"The release does not include a compatible CoInk installer." });
      return false;
    }
    dismissed = false;
    downloadActive = true;
    ...
    try {
      ...
      downloadedPath = await downloader({
        ...
        userAgent:`CoInk/${currentVersion}`,
        onProgress:progress => {
      ...
      downloadActive = false;
      downloadController = null;
      logger.warn?.(`CoInk update download failed: ${error.message || error}`);
      publish("error", {
        visible:true,
  ...
  async function install() {
    ...
    try {
      ...
      logger.warn?.(`CoInk could not install the downloaded update: ${error.message || error}`);
      publish("error", {
        visible:true,
