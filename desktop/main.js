...
const ROOT = path.resolve(__dirname, ".."),
  ...
  WINDOW_ICON = path.join(ROOT, "build", "icons", "coink.png"),
  HELP_URL = "https://github.com/hridyanshuucsd/CoInk#quick-start",
...
async function showLanAccessNotice(window) {
  ...
  const result = await dialog.showMessageBox(window, {
    type:"info",
    title:"CoInk on your local network",
    message:"CoInk is available to devices on your local network.",
    detail:`Open this address on another device:\n\n${currentLanUrls.join("\n")}\n\nKeep CoInk open, and use this only on a trusted network.`,
...
function startServer(configuration) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => fail(new Error("CoInk server did not become ready.")), 10000);
    try {
...
function macUpdateMenu(state) {
  ...
  if (state.status === "available") {
    return [{
      ...
      submenu:[
        { label:`CoInk${version} is available`, enabled:false },
        ...noteItems,
  ...
  if (state.status === "downloading") {
    return [{
      ...
      submenu:[
        { label:`Downloading CoInk${version}...`, enabled:false },
  ...
  if (state.status === "ready") {
    return [{
      ...
      submenu:[
        { label:`CoInk${version} is ready`, enabled:false },
        ...noteItems,
        ...(noteItems.length ? [{ type:"separator" }] : []),
...
function registerIpc() {
  ...
  ipcMain.handle("penecho:install-cli", async (event, provider) => {
    ...
    if (!fromCanvas(event) && !fromSetup) return { ok:false, error:"CLI installation is available only in the CoInk desktop application." };
...
async function bootstrap() {
  ...
  try {
    ...
    await dialog.showMessageBox({
      type:"error",
      title:"CoInk could not start",
      message:"CoInk could not start its local canvas service.",
...
if (gotLock) {
  ...
  app.whenReady().then(bootstrap).catch(error => {
    void dialog.showErrorBox("CoInk startup failed", error.message || String(error));
    app.quit();
  });
  app.on("activate", () => {
