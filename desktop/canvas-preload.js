...
function installWindowsBanner() {
  ...
  function render(state) {
    ...
    if (state.status === "available") {
      ...
      title.textContent = `CoInk${version} is ready`;
      detail.textContent = "Install the downloaded update and restart CoInk.";
      primaryButton.textContent = "Install";
    } else if (state.status === "installing") {
      title.textContent = `Installing CoInk${version}...`;
      detail.textContent = "CoInk will restart when installation finishes.";
      primaryButton.hidden = true;
      closeButton.hidden = true;
    } else if (state.status === "checking") {
      title.textContent = "Checking for CoInk updates...";
      primaryButton.hidden = true;
      notesButton.hidden = true;
    } else if (state.status === "up-to-date") {
      title.textContent = `CoInk v${state.currentVersion} is up to date`;
      primaryButton.hidden = true;
      notesButton.hidden = true;
    } else {
      title.textContent = "CoInk update failed";
