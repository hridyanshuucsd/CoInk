...
async function createRichUi(options = {}) {
  ...
  return {
    ...
    header(title, breadcrumb, detail = "") {
      ...
      output.write(`${ansi("1;36", "CoInk Configuration")}  ${ansi("2", breadcrumb)}\n`);
...
async function chooseModel(ui, provider, configuration) {
  ...
  const choices = provider === PROVIDERS.kimi
    ? [
        ...
        ...(current ? [{ name:`Current CoInk model: ${current}`, value:current }] : []),
    ...
    ? [
        ...
        { name:"gpt-5.6-sol (recommended)", value:"gpt-5.6-sol", description:"Best current choice for CoInk." },
    ...
    : [
        ...
        { name:"claude-opus-4-6", value:"claude-opus-4-6", description:"Supported, but CoInk quality may be lower." },
        { name:"Sonnet alias", value:"sonnet", description:"Supported, but CoInk quality may be lower." },
...
async function configureCodex(ui, configuration, options) {
  ui.header("Codex CLI", "Main menu  ›  LLM source  ›  Codex CLI",
    "Use GPT-5.5 or newer. gpt-5.6-sol is recommended and supports CoInk's medium default plus higher effort levels.");
...
async function configureSettings(ui, configuration, options) {
  ...
  const legacyTimeout = cleanText(env.CODEX_CLI_TIMEOUT_SECONDS || env.CLAUDE_CLI_TIMEOUT_SECONDS),
    ...
    hostChoice = await ui.select("Listening interface", uniqueChoices([
      { name:"All network interfaces (LAN access)", value:"0.0.0.0", description:"Reach CoInk from this computer and the local network." },
