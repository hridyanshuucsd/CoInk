...
function parseRefineModelText(text) {
  const stablePrefix = "CoInk Refine stable context (JSON; cacheable across edits of this target):\n",
    filesIntroduction = "\n\nCoInk virtual files follow.",
  ...
  const currentStart = cursor + currentPrefix.length,
    retryPrefix = "\n\nCoInk Refine retry instruction:\n",
...
function startServer(env) {
  ...
  return new Promise((resolve, reject) => {
    ...
    child.stdout.on("data", chunk => {
      ...
      const match = stdout.match(/CoInk: http:\/\/[^:]+:(\d+)/);
...
test("canvas shares ten persistent API and CLI connections without a server-wide selection", { timeout:10000 }, async () => {
  ...
  try {
    ...
    assert.doesNotMatch(missingKimiBody.error, /Install it|restart CoInk/);
...
test("two clients independently route requests through the shared connection list", { timeout:10000 }, async () => {
  ...
  try {
    ...
    const clientA = fetch(`${origin}/api/ai/command`, { method:"POST", headers:{ "Content-Type":"application/json", Accept:"application/x-ndjson", "X-CoInk-Connection":"default" }, body:JSON.stringify(validPayload()) }),
      clientB = fetch(`${origin}/api/ai/command`, { method:"POST", headers:{ "Content-Type":"application/json", Accept:"application/x-ndjson", "X-CoInk-Connection":connection.id }, body:JSON.stringify(validPayload()) });
    ...
    const deletedSelection = await fetch(`${origin}/api/ai/command`, { method:"POST", headers:{ "Content-Type":"application/json", "X-CoInk-Connection":connection.id }, body:JSON.stringify(validPayload()) });
...
test("a device can select a shared CLI connection while the server default remains API", { timeout:20000 }, async () => {
  ...
  try {
    ...
    const response = await fetch(`${origin}/api/ai/command`, { method:"POST", headers:{ ...headers, "X-CoInk-Connection":connection.id }, body:JSON.stringify(validPayload()) });
...
test("process-lifetime PIN gates the Canvas and local APIs, then clears on restart", { timeout:30000 }, async () => {
  ...
  try {
    ...
    const authenticatedPinRequest=await fetch(`${running.origin}/api/ai/command`,{
      method:"POST",
      headers:{"Content-Type":"application/json",Origin:running.origin,"X-CoInk-Session":setupBody.accessSessionToken},
      body:"{}",
    });
  ...
  try {
    ...
    const authenticatedOpenRequest=await fetch(`${running.origin}/api/ai/command`,{
      method:"POST",
      headers:{"Content-Type":"application/json",Origin:running.origin,"X-CoInk-Session":openBody.accessSessionToken},
      body:"{}",
    });
...
test("open Codex mode keeps its original same-origin request behavior", { timeout: 20000 }, async () => {
  ...
  try {
    ...
    const headerAuthorized=await fetch(`${origin}/api/ai/command`,{method:"POST",headers:{"Content-Type":"application/json",Origin:origin,"X-CoInk-Session":accessStatus.accessSessionToken},body:"{}"});
...
test("PIN authentication leaves the API provider request behavior unchanged", { timeout: 20000 }, async () => {
  ...
  try {
    ...
    const remote = await httpRequest(origin,{method:"POST",pathText:"/api/ai/command",headers:{Host:"my-pc:3888",Origin:"http://my-pc:3888","X-CoInk-Session":setupBody.accessSessionToken,"Content-Type":"text/plain","Content-Length":Buffer.byteLength(body)},body});
...
test("shared CoInk server canvases support authorized metadata-first CRUD", { timeout:20000 }, async () => {
  const stateDir=testStateDir({}),
...
test("shared CoInk server canvases retain up to one hundred widgets, images, and animations", { timeout:20000 }, async () => {
  const stateDir=testStateDir({}),
    {child,origin}=await startServer(serverEnv({PENECHO_STATE_DIR:stateDir})),
    mutationHeaders={"Content-Type":"application/json",Origin:origin},
...
test("shared server canvases obey the process-lifetime CoInk PIN session", { timeout:20000 }, async () => {
  const {child,origin}=await startServer(serverEnv({PENECHO_TEST_OPEN_ACCESS:"0"})),
  ...
  try {
    ...
    const setup=await fetch(`${origin}/api/local-access/setup-pin`,{
      ...
      sessionHeaders={"X-CoInk-Session":setupBody.accessSessionToken},
...
test("widget host CSP permits on-demand HTTPS resources inside the isolated widget", { timeout:10000 }, async () => {
  ...
  try {
    ...
    const sandboxedWidgetData = await fetch(`${origin}/api/widget-fetch`, {
      method:"POST",
      headers:{ "Content-Type":"application/json", "X-CoInk-Session":accessSession },
...
test("local plugin discovery is constrained and widget prompting is conditional", () => {
  ...
  assert.match(source, /Return only one compact final JSON object needed by CoInk[\s\S]*?Omit drafts, reasoning, progress or status updates, alternatives, duplicate objects, Markdown, and any wrapper text/);
...
test("local CLI requests from different Canvas clients do not supersede each other", { timeout: 20000 }, async () => {
  ...
  try {
    const base = { "Content-Type":"application/json", Origin:origin },
      clientA = fetch(`${origin}/api/ai/command`, { method:"POST", headers:{ ...base, "X-CoInk-Client":"123e4567-e89b-12d3-a456-426614174000" }, body:JSON.stringify(validPayload()) }),
      clientB = fetch(`${origin}/api/ai/command`, { method:"POST", headers:{ ...base, "X-CoInk-Client":"223e4567-e89b-12d3-a456-426614174000" }, body:JSON.stringify(validPayload()) }),
...
test("static page keeps strict styles while allowing the pinned MathJax CDN", () => {
  ...
  assert.doesNotMatch(app, /newClientRequestId|X-CoInk-Client-Request|X-CoInk-Replaces/);
