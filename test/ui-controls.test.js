...
test("plugin manager is a centered dynamic catalog with General HTML and bundled local plugins", () => {
  ...
  assert.match(app, /function authenticatedApiHeaders\([\s\S]*?X-CoInk-Session/);
  ...
  assert.match(app, /function authenticatedApiHeaders\([\s\S]*?X-CoInk-Client/);
  assert.match(app, /function aiRequestHeaders\([\s\S]*?X-CoInk-Connection/);
...
test("live widgets use native canvas chrome, state-aware iframe gestures, and three resize modes", () => {
  ...
  assert.match(messageHandler, /penecho-widget-snapshot-error[\s\S]*?console\.warn\("CoInk widget snapshot failed:"/);
...
test("canvas history clearly separates device-only and shared CoInk server storage", () => {
  const html = read("public/index.html"), app = read("public/app.js"), css = read("public/style.css"), zh = read("public/locales/zh.js");
  ...
  for (const key of ["saveLocation", "storageThisDevice", "storageCoInkServer", "storageThisDeviceDescription", "storageCoInkServerDescription"]) {
    assert.match(app, new RegExp(`${key}:`));
    assert.match(zh, new RegExp(`${key}:`));
  }
