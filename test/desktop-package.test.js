...
test("desktop shell and Forge config keep the renderer isolated and package native assets", () => {
  ...
  assert.match(settings, /result\.timedOut[\s\S]*?Connection test timed out[\s\S]*?still launch CoInk and enter the canvas/);
  assert.match(settings, /连接测试超过 30 秒，你仍然可以启动 CoInk 进入画布/);
  ...
  assert.match(html, /CoInk is a Kimi 2026 Open Source Partner/);
  assert.match(settings, /CoInk 是 Kimi 2026 开源合作伙伴/);
...
test("desktop updates resolve published GitHub Releases for each packaged target", async () => {
  assert.equal(expectedAssetName("darwin", "arm64", "0.7.1"), "CoInk-0.7.1-mac-arm64.zip");
  assert.equal(expectedAssetName("win32", "x64", "0.7.1"), "CoInk-Setup-0.7.1-win-x64.exe");
  ...
  assert.equal(requests[0].options.headers["User-Agent"], "CoInk/0.6.0");
...
test("unsigned desktop updater accepts only exact CoInk release assets", () => {
  const release = {
    version:"0.7.2",
    assets:[
  ...
  assert.equal(releaseAsset(release, "win32", "x64").name, "CoInk-Setup-0.7.2-win-x64.exe");
  assert.equal(macBundlePath("/Applications/CoInk.app/Contents/MacOS/CoInk"), "/Applications/CoInk.app");
  assert.equal(macBundlePath("/tmp/CoInk"), "");
});
...
test("unsigned desktop update download reports progress and verifies GitHub SHA-256", async () => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), "penecho-update-download-")),
    destination = path.join(directory, "CoInk-0.7.2-mac-arm64.zip"),
    content = Buffer.from("unsigned CoInk update"),
...
test("unsigned macOS updater validates the extracted app and schedules an atomic replacement", async () => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), "penecho-mac-install-")),
    target = path.join(directory, "Applications", "CoInk.app"),
    executable = path.join(target, "Contents", "MacOS", "CoInk"),
    archive = path.join(directory, "CoInk-0.7.2-mac-arm64.zip"),
    commands = [],
    spawns = [],
    app = {
...
test("unsigned Windows updater launches the downloaded Squirrel Setup silently", async () => {
  ...
  assert.equal(await installDownloadedUpdate({
    app,
    platform:"win32",
    downloadedPath:"C:\\Temp\\CoInk-Setup-0.7.2-win-x64.exe",
    spawnImpl:(command, args, options) => {
...
test("desktop CLI setup uses official installers without requiring npm", () => {
  const options = { platform:"darwin", home:"/Users/example", stateDir:"/Users/example/Library/Application Support/CoInk" },
  ...
  assert.equal(kimiPath, "/Users/example/Library/Application Support/CoInk/tools/kimi/bin/kimi");
  assert.equal(codexPath, "/Users/example/Library/Application Support/CoInk/tools/codex/bin/codex");
  assert.equal(claudePath, "/Users/example/.local/bin/claude");
  assert.equal(kimi.command, "/bin/bash");
  assert.equal(kimi.env.KIMI_INSTALL_DIR, "/Users/example/Library/Application Support/CoInk/tools/kimi");
