...
const ICON_SOURCE = path.join(ROOT, "build", "icons", "coink-1024.png");
...
async function writeSplash(target) {
  ...
  const temporary = `${target}.coink.png`;
  await sharp({
...
function configureIosNetwork() {
  ...
  info.NSLocalNetworkUsageDescription = "Connect to a CoInk server on your local network.";
  fs.writeFileSync(plistPath, plist.build(info));
}

function ensurePlatform(platform) {
  fs.copyFileSync(ICON_SOURCE, path.join(MOBILE_ROOT, "web", "coink-mark.png"));
...
function copyArtifact(source, extension, suffix) {
  ...
  const target = path.join(RELEASE_DIR, `CoInk-${APP_PACKAGE.version}-${suffix}.${extension}`);
...
async function buildIos() {
  ...
  configureIosNetwork();
  cap("sync", "ios");
  const archiveRoot = path.join(RELEASE_DIR, "ios", "CoInk.xcarchive");
  ...
  fs.cpSync(appPath, path.join(payloadRoot, "CoInk.app"), { recursive:true });
  const ipa = path.join(RELEASE_DIR, `CoInk-${APP_PACKAGE.version}-ios-unsigned.ipa`);
