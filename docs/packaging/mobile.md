# CoInk mobile packaging

CoInk mobile is a Capacitor client for Android and iOS. It connects to an existing CoInk Node.js service instead of copying API keys, CLI providers, or Cloud device credentials into the phone.

## Runtime model

1. Start CoInk on a computer or HTTPS server that the phone can reach.
2. For private LAN use, start the service with LAN listening enabled and open the port in the host firewall.
3. Open the mobile app and enter the complete CoInk address, for example `http://192.168.1.20:3888`.
4. The WebView navigates to that address. The canvas and every `/api/*` request then share the server origin and use the existing CoInk security checks.

The app remembers the last address in local WebView storage. It does not store the server's API key. HTTP is enabled so private LAN servers work, but an internet-facing CoInk service must use HTTPS.
...
# Produces release/mobile/CoInk-<version>-android-debug.apk by default.
npm run mobile:apk

# Produces release/mobile/CoInk-<version>-ios-unsigned.ipa.
npm run mobile:ipa
```
