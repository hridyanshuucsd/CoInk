...
1. CoInk opens the graphical setup page.
...
- macOS: `~/Library/Application Support/CoInk`
- Windows: `%APPDATA%\CoInk`
...
- macOS downloads the matching ZIP, validates its CoInk bundle ID and version, then replaces the installed `.app` after the running process exits. CoInk must be installed in a user-writable location.
